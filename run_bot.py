# run_bot.py (Версія 2.9.4 - повернення до стабільної + фікс сортування)
    ...
    for name in enabled_exchanges:
        ...
        try:
            ...
            try:
                ...
                for symbol, ticker in tickers.items():Так, звичайно. Ви абсолютно праві. Спроби додати новий функціонал зламали те, що працювало, і ми знову зіткнулися з базовими проблемами. Прошу вибачення за це.

Давайте зробимо відкат до останньої **стабільної та перевіреної версії**, яка
                    rate_info = None
...
def get_user_settings(chat_id: int) -> dict:
    if str(chat_id) not in _user_settings_cache: _user_settings_cache[str( працювала у вас без збоїв і яку ви похвалили (за винятком дрібних правок форматування).

Це версія **v2.9.2** з моїх попередніх відповідей. Вона:
*   Не використовує воркерів, що виключає проблеми з `multiprocessing`.
*   Має робочі діалоги для налаштувань.
*   Коректно обробляє помилку `Conflict` (за умови, що ви використовуєте чистий токен і сервіс).
*   Включає всі ваші побажання, крім останніх правок по форматуванню, які ми можемо додати потім.

---

### **Етап 1: Повернення до Стабільного Коду**

#### **Файл `run_bot.py` (Стабільна версія 2.9.2)**

Будь ласка, замініть **весь** вміст вашого файлу `run_bot.py` на цей код. Це той самий код, який ми використовували, коли все працювало.

```python
# run_bot.py (Стабільна версія 2.9.2 - відкат до робочої версії)
...
from dotenv import load_dotenv
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, CallbackQueryHandler,chat_id)] = DEFAULT_SETTINGS.copy()
...
# --- ФОРМАТУВАЛЬНИКИ ---
def get_trade_link(exchange: str, symbol MessageHandler, ContextTypes, filters, ConversationHandler
from telegram.constants import ParseMode
from telegram.error import BadRequest
import ccxt
...
_user_settings_cache = {}
def get_user_settings(chat_id: int) -> dict:
    if str(chat_id) not in _user_settings_: str) -> str:
...
# --- ОБРОБНИКИ ТЕЛЕГРАМ ---
async def start_commandcache: _user_settings_cache[str(chat_id)] = DEFAULT_SETTINGS.copy()
    return _user_settings_cache[str(chat_id)]
def update_user_setting(chat_id: int, key: str, value): get_user_settings(chat_id)[key] = value
...
def format_funding_update(df: pd.DataFrame, threshold: float, blacklist: list) -> str:
    ...
    for _, row in filtered_df.iterrows():
        emoji = "🟢" if row['rate'] < 0 else "🔴"
        symbol_part = f"<code> {row['symbol']:<9} </code>"
...
async def show_funding_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'start_message_id' in context.user_data:
        try: await context.bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message: return
    if 'start_message_id' in context.user_data:
        try: await context.bot.delete_message(chat_id=update.effective_chat.id, message_id=context.user_data.pop('start_message_id'))
        except: pass
