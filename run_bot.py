# run_bot.py (Версія з виправленням AttributeError та новими кнопками)
...
# --- РОБОТА З НАЛАШТУВАННЯМИ (без змін) ---
...
# НОВА КЛАВІАТУРА для пошуку по тикеру
def get_ticker_menu_keyboard(ticker: str):
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Оновити", callback_data=f"refresh_ticker_{ticker}"), InlineKeyboardButton("↩️ Назад", callback_data="refresh")]])

# --- ФОРМАТУВАЛЬНИКИ (без змін) ---
...
async def refresh_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ВИПРАВЛЕНО: отримуємо chat_id з повідомлення, в якому була кнопка
    chat_id = query.message.chat.id
    logger.info(f"Отримано 'refresh' для чату {chat_id}")
...
async def settings_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ВИПРАВЛЕНО: отримуємо chat_id з повідомлення, в якому була кнопка
    chat_id = query.message.chat.id
    logger.info(f"Отримано 'settings_menu' для чату {chat_id}")
    await query.answer()
    settings = get_user_settings(chat_id)
...
async def close_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    # Замість видалення, просто оновлюємо повідомлення на головний звіт
...
async def ticker_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ДОДАНО: кнопки під повідомленням
...
async def refresh_ticker_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"ПОМИЛКА в refresh_ticker_callback: {e}", exc_info=True)

# ... (інші обробники: exchange_menu, toggle_exchange, set_threshold) ...
...
async def set_threshold_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    settings = get_user_settings(query.message.chat.id)
    await query.edit_message_text(f"Поріг: <b>{settings['threshold']}%</b>.\n\nНадішліть нове значення.", parse_mode=ParseMode.HTML, reply_markup=get_back_to_settings_keyboard())
...
async def set_threshold_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("Некоректне значення.")
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(refresh_ticker_callback, pattern="^refresh_ticker_")) # НОВИЙ ОБРОБНИК
    application.add_handler(conv_handler)
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, ticker_message_handler))
    
    logger.info("Бот запускається (версія з виправленням AttributeError)...")
