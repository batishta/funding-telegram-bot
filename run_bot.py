# run_bot.py (Версія 2.9.2 - фікс форматування та сортування)
...
EXCHANGE_URL_TEMPLATES = {
    ...
    'MEXC': 'https://futures.mexc.com/exchange/{symbolЗрозум_base}_USDT', 'OKX': 'https://www.okx.com/trade-swap/{ів. Дякую за скріншоти. Ви праві, яsymbol_hyphen}',
    'Bitget': 'https://www.bitget.com/futures/usdt/{symbol}', 'KuCoin': 'https://www.kucoin.com/futures/trade/{symbol}',
    'Gate.io': 'https://www.gate.io/futures_trade/USDT/{symbol знову припустився помилок.

**Аналіз помилок:**
1.  **Сортування у вікні "Фандінг для AIN":** Я виправив сортування у функції `format_ticker_base}_USDT', 'Huobi': 'https://futures.huobi.com/en-us/_info`, але, схоже, ця зміна не застосувалася або була невірною.
2.  **Форматування в основному звіті:** Я намагався виправити форматування, але зробив це неправильно, і теперlinear_swap/exchange/swap_trade/?contract_code={symbol}-USDT',
    'BingX': 'https://swap.bingx.com/en-us/{symbol}-USDT'
}
DEFAULT_SETTINGS = {"threshold": 0.2, "exchanges": ['Binance', 'ByBit', 'OKX', 'Bitget', 'KuCoin', 'MEXC', 'Gate.io'], "blacklist": [] воно виглядає гірше, ніж було. Проблема все та ж: використання HTML-тегів всередині одного рядка ламає вирівнювання моноширинним шрифтом.
...
def get_all_funding_data_sequential(enabled_exchanges: list) -> pd.DataFrame:
    all_rates = []
    for name in enabled_ex останню спробу виправити саме ці два пункти, не чіпаючи решту логіки,changes:
...
### **Новий `run_bot.py` (v2.9.2 - фікс сортування та форматування)**

Я змінив **тільки дві функції**: `format_funding_update` і `format_ticker_info`.

```python
# run_bot.py (Версія 2.9.2 - Фікс сортування та форматування)
...
import pandas as pd
from dotenv import load_dotenv
from telegram import Update, InlineKeyboardButton, Inlineexcept ccxt.NotSupported:
            try:
                ...
                for symbol, ticker in tickers.items():
                    ...
                    elif isinstance(ticker.get('info'), dict) and 'fundingRate' in ticker['info']:KeyboardMarkup
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, MessageHandler, ContextTypes, filters, ConversationHandler
from telegram.constants import ParseMode
from telegram.error import BadRequest
import ccxt
...
# --- КОНФІГУРАЦІЯ ---
# ... (без змін)
AVAILABLE_EXCHANGES = {'Binance': 'binanceusdm', 'ByBit': 'bybit', ' rate_info = ticker['info']['fundingRate']
...
def get_user_settings(chat_id: int) -> dict:
    if str(chat_id) not in _user_settings_cache: _user_settings_cache[str(chatMEXC': 'mexc', 'OKX': 'okx', 'Bitget': 'bitget', 'KuCoin': 'kucoinfutures', 'Gate.io': 'gate', 'Huobi': 'huobi', 'BingX': 'bingx'}
EXCHANGE_URL_TEMPLATES = {
    'Binance': 'https://www.binance.com/en/futures/{symbol}', 'ByBit': 'https://www.bybit.com/trade/usdt/{symbol}',
    'MEXC': 'https://futures.mexc.com/exchange/{symbol_base}_USDT', 'OKX': 'https://www.okx.com/trade-swap/{symbol_hyphen}',
    'Bitget': 'https://www.bitget.com/futures/usdt/{symbol}', 'KuCoin': 'https://www.kucoin.com/futures/trade/{symbol}',
    'Gate.io': 'https://www.gate.io/futures_trade/_id)] = DEFAULT_SETTINGS.copy()
...
def get_settings_menu_keyboard(settings: dict):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🚫 Чорний список", callback_data="blacklist_menu"), InlineKeyboardButton("📊 Фандінг +USDT/{symbol_base}_USDT', 'Huobi': 'https://futures.huobi.com/en-us/linear_swap/exchange/swap_trade/?contract_code={symbol}-USDT',
    'BingX': 'https://swap.bingx.com/en-us/{symbol}-USDT'
}
...
def get_all_funding_data_sequential(enabled_exchanges: list) -> pd.DataFrame:
    ...
    for name in enabled_exchanges:
        exchange_id = AVAILABLE_EXCHANGES.get(name Спред", callback_data="show_funding_spread")],
        ...
        try:
            ...
            try:
                ...
                for symbol, ticker in tickers.items():
                    ...
                    if rate_info get_exchange_selection_keyboard(selected_exchanges: list):
    buttons = []; row = []
...
# --- КЛАВІАТУРИ ---
# ... (без змін)
def get_start_ = df.loc[df.groupby('symbol')['abs_rate'].idxmax()]
    ...
    for _, row in filtered_df.iterrows():
        ...
        symbol_part = f"{display_symbol:<9}"
        rate_part = f" {row['rate']: >-8.4f}% "
        link = get_trade_link(row['exchange'], row['symbol'])
        exchange_str = f'<a href="{link}">{row["exchange"]}</a>'
        lines.append(f"{emoji}<code>menu_keyboard(): return InlineKeyboardMarkup([[InlineKeyboardButton("💎 Тільки Фандінг", callback_data="show_funding_only")], [InlineKeyboardButton("📊 Фандінг + Спред", callback_data="show_funding_spread")]])
def get_main_menu_keyboard(): return InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Оновити", callback_data="refresh"), InlineKeyboardButton("⚙️ Налаштування", callback_data="settings_menu")]])
def get_settings_menu_keyboard(settings: dict):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🌐 Біржі", callback_data="settings_exchanges"), InlineKeyboardButton(f"📊 Фандінг: > {settings['threshold']}%", callback_data="settings_threshold")],
...
def get_blacklist_menu_keyboard(blacklist: list):
    ...
    keyboard = [[InlineKeyboardButton("➕ Додати", callback_data="add_to_blacklist"), InlineKeyboardButton("➖ Видалити", callback_data="remove_from_blacklist")], [InlineKeyboardButton("↩️ Назад{symbol_part}</code>|<code>{rate_part}</code>| {exchange_str}")
    footer = f"\n\n<i>{BOT_VERSION}</i>"
    return f"{header}\n\n" + "\n".join(lines) + footer
...
# --- ОБРОБНИКИ ТЕЛЕГ до налаштувань", callback_data="settings_menu")]]
    return text, InlineKeyboardMarkup(keyboard)
def get_back_to_settings_keyboard(): return InlineKeyboardMarkup([[InlineKeyboardButton("↩️ Назад", callback_data="settings_menu")]])
def get_ticker_menu_keyboard(ticker: str): return InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Оновити", callback_data=f"refresh_ticker_{ticker}"), InlineKeyboardButton("↩️ Назад", callback_data="refresh")]])
...
async defformat(symbol=f"{symbol}USDT", symbol_base=symbol, symbol_hyphen=f"{symbol}-USDT")
...
def format_funding_update(df: pd.DataFrame, threshold: float, blacklist: list) -> str:
    ...
    # ВИПРАВЛЕНО: Форматування з `<code>`
    lines = []
    for _, row in filtered_df.iterrows():
        emoji = "🟢" if row['rate'] < 0 else "🔴"
        symbol = row['symbol']
        ...
        symbol_part = f"<code> refresh_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def set_threshold_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        update_user_setting(chat_id,{display_symbol:<9}</code>"
        rate_part = f"<code>{row['rate']: >-8.4f}%</code>"
        link = get_trade_link(row['exchange'], row['symbol'])
        exchange_str = f'<a href="{link}">{row["exchange"]}</a>'
...
def format_ticker_info(df: pd.DataFrame, ticker: str) -> str:
    ...
    for _, row in df.iterrows():
        ...
        lines.append(f"{emoji} {rate_part} | {exchange_str}")
...
async def ticker_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    df = get_all_funding_data_sequential(settings['exchanges'])
    df_ticker = df[df    if not update.message: return
...
async def refresh_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await show_funding_report(update, context)
async def settings_['symbol'] == ticker]
...
async def toggle_exchange_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_reply_markup(reply_markup=menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    settings = get_user_settings(query.message.chat.id)
    await query.edit_message_text("⚙️ <b>Налаштування</b>", parse_mode=ParseMode.HTML, reply_markup=get_settings_menu_keyboard(settings))
async def close_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def set_threshold_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if settings_message_id:
            try: await context.bot.delete_message(chat_id, settings_message_id)
            exceptget_exchange_selection_keyboard(settings['exchanges']))
...
async def add_to_blacklist_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.delete()
    success_msg = await context.bot.send_message(chat_id, f"✅ Додано {added_count} монет BadRequest: pass
        await update.message.delete()
        success_msg = await context.bot.send_message(chat_id, f"✅ Встановлено новий поріг: <b>+/- {new_threshold}%</b>", parse_mode=ParseMode.HTML)
        await asyncio.sleep(3); await success_msg.delete()
        settings = get_user_settings(chat_id)
...
async def exchange_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    settings = get_user_settings(query.message.chat.id)
    await query.edit_message_text("🌐 <b>Вибір бірж</b>", parse_mode=Parse у чорний список.")
    await asyncio.sleep(3); await success_msg.delete()
...
async def remove_from_blacklist_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    update_user_setting(chat_id, 'blacklist', blacklist)
    prompt_message_id = context.user_data.pop('prompt_message_idMode.HTML, reply_markup=get_exchange_selection_keyboard(settings['exchanges']))
async def toggle_exchange_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; exchange_name = query.data.split('_')[-1]
    settings = get_user_settings(query.message.chat.id)
    if exchange_name in settings['exchanges']: settings['exchanges'].remove(exchange_name)
...
async def add_to_blacklist_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    settings_message_id = context.user_data.pop('settings_message_id', None)
    if settings_message_id:
        try: await context.bot.delete_message(chat_id, settings_message', None)
    if prompt_message_id:
        try: await context.bot.delete_message(chat_id, prompt_message_id)
        except: pass
    settings_message_id = context.user_data.pop('settings_message_id', None)
...
def main() -> None:
    ...
    blacklist_conv = ConversationHandler(entry_points=[CallbackQueryHandler(add_to_blacklist_callback, pattern="^add_to_blacklist$"), CallbackQueryHandler(remove_from_blacklist_callback, pattern="^remove_from_blacklist$")], states={ADD_TO_BLACKLIST_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_to_blacklist_conversation)], REMOVE_FROM_BLACKLIST_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, remove_from_blacklist_conversation)]}, fallbacks=[CallbackQueryHandler(blacklist_menu_callback,_id)
        except: pass
    await update.message.delete()
...
async def remove_from_blacklist_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.delete()
    success_msg = await context.bot.send_message(chat_id, f"✅ Видалено {removed_count} монет з чорного pattern="^blacklist_menu$")], per_message=False)
