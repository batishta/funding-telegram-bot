...
EXCHANGE_URL_TEMPLATES = { 'Binance': 'https://www.binance.com/en/futures/{symbol}', 'ByBit': 'https://www.bybit.com/trade/usdt/{symbol}', 'MEXC': 'https://futures.mexc.com/exchange/{symbol_base}_USDT', 'OKX': 'https://www.okx.com/trade-swap/{symbol_hyphen}', 'Bitget': 'https://www.bitget.com/futures/usdt/{symbol}', 'KuCoin': 'https://www.kucoin.com/futures/trade/{symbol}', 'Gate.io': 'https://www.gate.io/futures_trade/USDT/{symbol_base}_USDT', 'Huobi': 'https://futures.huobi.com/en-us/linear_swap/exchange/swap_trade/?contract_code={symbol}-USDT', 'BingX': 'https://swap.bingx.com/en-us/{symbol}-USDT' }
    ...
    for name in enabled_exchanges:
        ...
        try:
            ...
            for symbol, data in funding_rates_data.items():
                if 'USDT' in symbol and data.get('fundingRate') is not None: all_rates.append({'symbol': symbol.split('/')[0], 'rate': data['fundingRate'] * 100, 'exchange': name})
            ...
            try:
                ...
                for symbol, ticker in tickers.items():
                    rate_info = ticker.get('fundingRate') or (isinstance(ticker.get('info'), dict) and ticker['info'].get('fundingRate'))
                    if rate_info is not None: all_rates.append({'symbol': symbol.split('/')[0], 'rate': float(rate_info) * 100, 'exchange': name})
...
def worker_process(task_queue: mp.Queue, result_queue: mp.Queue):
    logger.info("Воркер запущений і готовий до роботи.")
    while True:
        try:
            job_id, task_type, payload = task_queue.get()
            if task_type == 'STOP': break
            logger.info(f"Воркер отримав завдання '{task_type}' #{job_id[:6]}")
            if task_type == 'get_all':
                df = get_all_funding_data_sequential(payload)
                result_queue.put((job_id, df))
        except Exception as e:
            logger.error(f"Критична помилка у воркері: {e}", exc_info=True)
...
def get_settings_menu_keyboard(settings: dict): return InlineKeyboardMarkup([[InlineKeyboardButton("🌐 Біржі", callback_data="settings_exchanges"), InlineKeyboardButton(f"📊 Фандінг: > {settings['threshold']}%", callback_data="settings_threshold")], [InlineKeyboardButton("🚫 Чорний список", callback_data="blacklist_menu"), InlineKeyboardButton("📊 Фандінг + Спред", callback_data="show_funding_spread")], [InlineKeyboardButton("ℹ️ Довідка", url=HELP_URL), InlineKeyboardButton("↩️ Назад", callback_data="close_settings")]])
def get_blacklist_menu_keyboard(blacklist: list):
    text = "🚫 Ваш чорний список:\n" + ("<code>" + ", ".join(blacklist) + "</code>" if blacklist else "<i>Порожній</i>")
...
def get_exchange_selection_keyboard(selected_exchanges: list):
    ...
    buttons.append([InlineKeyboardButton("↩️ Назад", callback_data="settings_menu")]); return InlineKeyboardMarkup(buttons)

# --- ФОРМАТУВАЛЬНИКИ ---
def get_trade_link(exchange: str, symbol: str) -> str:
    template = EXCHANGE_URL_TEMPLATES.get(exchange)
    return template.format(symbol=f"{symbol}USDT", symbol_base=symbol, symbol_hyphen=f"{symbol}-USDT") if template else ""
def format_funding_update(df: pd.DataFrame, threshold: float, blacklist: list) -> str:
    ...
    df = df[~df['symbol'].isin(blacklist)]; df['abs_rate'] = df['rate'].abs()
    ...
    for _, row in filtered_df.iterrows():
        ...
        display_symbol = (row['symbol'][:7] + '..') if len(row['symbol']) > 8 else row['symbol']
        symbol_part = f"<code>{display_symbol:<9}</code>"; rate_part = f"<code> {row['rate']: >-8.4f}% </code>"
        exchange_str = f'<a href="{get_trade_link(row["exchange"], row["symbol"])}">{row["exchange"]}</a>'
...
def format_ticker_info(df: pd.DataFrame, ticker: str) -> str:
    ...
    header = f"🪙 <b>Фандінг для <code>{html.escape(ticker.upper())}</code></b>"
    # ВИПРАВЛЕНО: Сортування за спаданням ставки
...
async def process_funding_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"Завдання #{job_id[:6]} в черзі. Очікую на результат...")
    task_queue.put((job_id, 'get_all', settings['exchanges']))
    
    result_df = None
    for _ in range(60): # Чекаємо до 60 секунд
        try:
            job_result_id, df = result_queue.get_nowait()
            if job_result_id == job_id: result_df = df; break
            else: result_queue.put((job_result_id, df))
        except Empty: await asyncio.sleep(1)
    ...
    if result_df is not None:
        ...
        await query.edit_message_text("😔 Воркер не відповів вчасно. Спробуйте оновити.", reply_markup=get_main_menu_keyboard())
# ... (решта обробників без змін, окрім close_settings)
...
async def settings_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    await query.edit_message_text("⚙️ <b>Налаштування</b>", parse_mode=ParseMode.HTML, reply_markup=get_settings_menu_keyboard(get_user_settings(query.message.chat.id)))
async def close_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await process_funding_request(update, context) # Повертаємо головний звіт
# ... (весь код діалогів і решти обробників залишається без змін)
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(process_funding_request, pattern="^show_funding_only$"))
